...
void writeInFileConvertedVoiceMatching(ifstream& finThree, ifstream& finTwo, ofstream& foutThree, ifstream& fin,
                                       ofstream& fout, string filename, string endOne)
{
    finThree.open("./eng/" + filename.substr(0, filename.length() - 4) + endOne);
...
void runConvert(string filename)
{
    ...
    if (foutThree.is_open()) {
        ...
        foutThree.close();
        ifstream finCopy("./result/" + filename.substr(0, filename.length() - 4) + endOne);
        ofstream foutCopy("./result/" + filename.substr(0, filename.length() - 4) + endTwo);
    ...
    if (foutThree2.is_open()) {
        ...
        foutThree2.close();
        ifstream finCopy("./result/" + filename.substr(0, filename.length() - 4) + endOne2);
        ofstream foutCopy("./result/" + filename.substr(0, filename.length() - 4) + endTwo2);
        foutCopy << finCopy.rdbuf();
        finCopy.close();
        foutCopy.close();
    }
