...
void formatFileHina(const string& filename, const string& end);
...
void writeInFileConvertedDate(ifstream& fin, ifstream& finTwo, ofstream& fout, string buff)
{
    for (int i = 0; !fin.eof(); ++i) {
        ...
        if (isCharacterName(buff)) {   
            ...
            cin >> buff;
        } else if ((buff.find(" NULL, \"") != -1 || buff.find("\tNULL, \"") != -1) && buff.find(", NULL, \"") == -1) {
            string s;
            for (int j = 0; !finTwo.eof(); ++j) {
                getline(finTwo, s);
                if ((s.find(" NULL, \"") != -1 || s.find("\tNULL, \"") != -1) && s.find(", NULL, \"") == -1) {
                    break;
                }
            }
...
void writeInFileConvertedVoiceMatching(ifstream& finThree, ifstream& finTwo, ofstream& foutThree, ifstream& fin,
                                       ofstream& fout, string filename, string endOne)
{
    finThree.open("./eng/_z" + filename.substr(1, filename.length() - 5) + endOne);
    if (!foutThree.is_open()) {
        foutThree.open("./temp/_z" + filename.substr(1, filename.length() - 5) + endOne);
        string str;
        for (int k = 0; k < 4; ++k) {
...
void formatFileHina(const string& filename, const string& end)
{
    ifstream fin("./temp/_z" + filename.substr(1, filename.length() - 5) + end);
    ofstream fout("./result/_z" + filename.substr(1, filename.length() - 5) + end);
    string buff;
    for (int i = 0; i < 4; ++i) {
        getline(fin, buff);
        fout << buff << endl;
...
void runConvert(string filename)
{
    ...
    fout.close();
    if (foutThree.is_open()) {
        formatFileHina(filename, endOne);
        foutThree.close();
        ifstream finCopy("./result/_z" + filename.substr(1, filename.length() - 5) + endOne);
        ofstream foutCopy("./result/_z" + filename.substr(1, filename.length() - 5) + endTwo);
        foutCopy << finCopy.rdbuf();
        finCopy.close();
        foutCopy.close();
    }
    if (foutThree2.is_open()) {
        formatFileHina(filename, endOne2);
        foutThree2.close();
        ifstream finCopy("./result/_z" + filename.substr(1, filename.length() - 5) + endOne2);
        ofstream foutCopy("./result/_z" + filename.substr(1, filename.length() - 5) + endTwo2);
        foutCopy << finCopy.rdbuf();
        finCopy.close();
        foutCopy.close();
    }
