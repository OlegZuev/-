...
DWORD WINAPI badguys(LPVOID)
{
	...
	WaitForSingleObject(startevt, INFINITE);
...
int main()
{
	...
	// arr.emplace_back(5, 7);
	// int a = 5;
	// HANDLE hShapshot;
	// PROCESSENTRY32 entry;
 //
	// hShapshot = CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, 0);
 //    Process32First(hShapshot, &entry);
 //    do {
	// 	if (strcmp(entry.szExeFile, "Project12.exe") == 0) {
	// 		std::cout << entry.szExeFile << " <---------- here"<< std::endl;
	// 	} else {
	// 		std::cout << entry.szExeFile << std::endl;
	// 	}
	// } while (Process32Next(hShapshot, &entry));
 //
	// getchar();
 //
	// HANDLE semaphore = CreateSemaphoreA(nullptr, 1, 5, "GridCounter");
 //
	// return 0;
	...
	cinfo.dwSize = 100; // иначе не исчезает курсор 
	...
	bulletsem = CreateSemaphore(NULL, 5, 5, NULL);
